...
import { createSite, getSiteDetail, updateSite } from '../../../../actions/siteActions';
import moment from 'moment';
import PrintScreen from './PrintScreen';
import Loader from '../../../elements/Loader';
import { toast } from 'react-toastify';

const SiteDetail = ({ isDetail, match }) => {
    ...
    const { siteValidation, siteDetail, createNewSite, updateExistingSite } = siteDetails
    ...
    useEffect(() => {
        if (isDetail && updateExistingSite) {
            setIsLoading(updateExistingSite.loading)
            if (updateExistingSite.error) {
                toast.error(updateExistingSite.error, {
                    position: toast.POSITION.TOP_RIGHT
                })
            }   else if (updateExistingSite.success == true) {
                history('/dashboard')
            }
        } else if (createNewSite) {
            setIsLoading(createNewSite.loading)
            if (createNewSite.error) {
                toast.error(createNewSite.error, {
                    position: toast.POSITION.TOP_RIGHT
                })
            }   else if (createNewSite.success == true) {
                history('/dashboard')
            }
        }
    },[createNewSite, updateExistingSite])

    useEffect(() => {
        if (isDetail) {
            if (siteDetail.siteInfo) {
                setSiteName(siteDetail.siteInfo.siteName)
                setGpsRange(siteDetail.siteInfo.gpsRange)
                setStartDate(moment(siteDetail.siteInfo.periodStart, 'YYYY/MM/DD').toDate())
                setEndDate(moment(siteDetail.siteInfo.periodEnd, 'YYYY/MM/DD').toDate())
                setBrowseFrom(siteDetail.siteInfo.browseTimeFrom)
                setBrowseTo(siteDetail.siteInfo.browseTimeTo)
                setCode(siteDetail.siteInfo.qrCodeValue)
                setMarkerPosition({
                    lat: parseFloat(siteDetail.siteInfo.latitude == null ? 37.7749 : siteDetail.siteInfo.latitude), 
                    lng: parseFloat(siteDetail.siteInfo.longitude == null ? -122.4194 : siteDetail.siteInfo.longitude)
                });
            }
        }
    },[siteDetail])
    ...
    return (
        <>
            ...
            <div x-data="lw" class="flex flex-col justify-evenly">
                    ...
                    <div class="flex-1 p-5 bg-white">
                        ...
                        <div className='flex flex-row mt-2'>
                            <div className='mr-4'>
                                ...
                                >
                                    {isDetail ? 
                                        <p>{t("Update Site")}</p> : <p>{t("Create Site.1")}</p>
                                    }
                                </button>
                            </div>
                            <div>
                                <button
                                    onClick={printScreen}
                                    type="submit"
                                    class="flex flex-row items-center w-full px-4 py-2 text-lg font-semibold text-white transition-colors duration-300 bg-cyan-500 rounded-md shadow hover:bg-cyan-600 focus:outline-none focus:ring-cyan-200 focus:ring-4"
                                >
                                    {t('Print QR')}
                ...
                {showModal ?  <PrintScreen closeFunction={printScreen} qrValue={code} /> : null }
            </div>
        </>
    )
}

const PlacesAutocomplete = ({ setMarkerPosition }) => {
    const {
        ready,
        value,
        setValue,
