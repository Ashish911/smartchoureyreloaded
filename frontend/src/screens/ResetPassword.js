...
import { resetPasswordByToken } from '../actions/userActions';
import { validateForReset } from '../util/util';
import { toast } from 'react-toastify';
import Loader from './elements/Loader';
import { USER_RESET_PASSWORD_RESET } from '../contsants/userConstants';

const ResetPassword = ({onchange, i18next}) => {
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        let validate = validateForReset(email, password, confirmPassword)
        if (validate == true) {
            dispatch(resetPasswordByToken(email, password, confirmPassword, token));
        }
    }

    useEffect(() => {
        if (resetPasswordInfo) {
            setIsLoading(resetPasswordInfo.loading)
            if (resetPasswordInfo?.error) {
                toast.error(resetPasswordInfo.error, {
                    position: toast.POSITION.TOP_RIGHT
                })
                dispatch({ type: USER_RESET_PASSWORD_RESET })
            } else if (resetPasswordInfo?.success == true) {
                history('/')
            }
        }
    }, [resetPasswordInfo])
            ...
            <div class="flex items-center min-h-screen p-4 bg-cyan-600 lg:justify-center">
                ...
                >
                    ...
                    <form action="#" class="flex flex-col space-y-5">
                        ...
                        <div class="flex flex-col space-y-1">
                            <label for="email" class="text-sm font-semibold text-gray-500 flex flex-start">{t('Password.1')}</label>
                            <input
                                type="password"
                                id="password"
                                value={password}
                        ...
                        <div class="flex flex-col space-y-1">
                            <label for="email" class="text-sm font-semibold text-gray-500 flex flex-start">{t('Confirm Password.1')}</label>
                            <input
                                type="password"
                                id="password"
                        ...
                        <div className="flex flex-col space-y-5">
                            ...
                            <span className="font-normal text-gray-500">{t("Back to")}</span>
                            <Link to={'/'} className="font-normal text-cyan-400">{t("Login.1")}</Link>
                            </span>
                        </div>
                    </form>
                    </div>
